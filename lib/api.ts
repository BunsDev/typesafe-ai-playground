...
export function validatePayload(value: unknown): RunPayload {
  ...
  if (
    ...
    Object.keys(value.questions).length > 100
  )
  ...
  for (const [key, q] of Object.entries(value.questions)) {
    if (
      !nonempty(key) ||
      Object.hasOwn(questions, key.trim()) ||
      !object(q) ||
      !nonempty(q.instructions) ||
      !["noul", "choice", "score"].includes(String(q.type))
    )
      throw Error(
        "Every question needs a unique name, valid type, and instructions.",
      );
    ...
    if (q.type === "choice") {
      if (
        ...
        Object.entries(q.criteria).some(
          ([k, v]) => !nonempty(k) || !nonempty(v),
        )
      )
        throw Error("Choice questions need at least two named candidates.");
      const entries = Object.entries(q.criteria).map(([k, v]) => [
        k.trim(),
        (v as string).trim(),
      ]);
  ...
  return {
    state: value.state,
    model: nonempty(value.model) ? value.model.trim() : "jev-latest",
    questions,
  };
}
export async function readBoundedBody(
  ...
  try {
    while (true) {
      ...
      if (total > limit) {
        await reader.cancel();
        throw Error("Body exceeds the size limit.");
      }
      chunks.push(value);
    }
  } finally {
