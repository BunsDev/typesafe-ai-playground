...
export async function fetchTranscript(
  ...
  async function read(url: string, init: RequestInit = {}) {
    ...
    if (!response.ok) {
      ...
      throw Error(
        "YouTube could not serve this public caption track. It may be unavailable or blocked.",
      );
    }
    return readBoundedBody(response.body, 2 * 1024 * 1024);
  }
  ...
  if (publicKey) {
    ...
    if (!marker)
      throw Error(
        "YouTube did not expose captions. The video may be private, restricted, or blocked.",
      );
    ...
    if (end < 0) throw Error("YouTube caption metadata is unavailable.");
    player = JSON.parse(html.slice(start, end));
  }
  const tracks =
    player?.captions?.playerCaptionsTracklistRenderer?.captionTracks;
  if (!Array.isArray(tracks))
    throw Error("No public captions available for this video.");
  ...
  if (!track)
    throw Error(
      "No existing caption track available. This prototype does not translate or transcribe.",
    );
  ...
  if (
    ...
    throw Error("Unsupported caption track URL.");
  ...
  if (!lines.length)
    throw Error(
      "YouTube returned no usable captions. This track may require authentication or be blocked.",
    );
  chunkCaptions(lines);
  return {
    videoId: id,
